...
from Foundation import NSMakePoint, NSMakeSize, NSObject, NSString, NSTimer, NSURL
...
class Editor(NSObject):
    ...
    def _rebuild_inspector(self):
        ...
        # спец-секция для «FPS выбранного окна»
        if key == "winfps":
            gy = 330
            self.insp.addSubview_(
                _label(NSMakeRect(6, 196, iw - 12, 18), "Окно для замера:", 11, True)
            )
            pop = NSPopUpButton.alloc().initWithFrame_(NSMakeRect(6, 218, iw - 12, 26))
            pop.addItemWithTitle_("— выбери окно —")
            self._win_list = self._list_windows()
            for w in self._win_list:
                pop.addItemWithTitle_(f"{w['name']} — {w['w']}x{w['h']}")
            pop.setTarget_(self)
            pop.setAction_(b"pickWindow:")
            self.insp.addSubview_(pop)

            ref = NSButton.alloc().initWithFrame_(NSMakeRect(6, 250, 150, 26))
            ref.setTitle_("Обновить список")
            ref.setBezelStyle_(1)
            ref.setTarget_(self)
            ref.setAction_(b"refreshWindows:")
            self.insp.addSubview_(ref)

            wf = self.mon.winfps
            status = "окно не выбрано"
            if wf is None:
                status = "недоступно на этой системе"
            elif wf.error:
                status = "нет доступа «Запись экрана»"
            elif wf.active and wf.target:
                status = f"меряю: {wf.target}"
            st = _label(NSMakeRect(6, 280, iw - 12, 34), status, 11)
            st.cell().setWraps_(True)
            self.insp.addSubview_(st)

            if wf is not None and wf.error:
                acc = NSButton.alloc().initWithFrame_(NSMakeRect(6, 300, iw - 12, 26))
                acc.setTitle_("Открыть доступ «Запись экрана»")
                acc.setBezelStyle_(1)
                acc.setTarget_(self)
                acc.setAction_(b"openScreenAccess:")
                self.insp.addSubview_(acc)
    ...
    def _list_windows(self):
        try:
            from winfps_mac import list_windows
            return list_windows()
    ...
    def pickWindow_(self, sender):
        ...
        if self.mon.winfps is None or idx < 1 or idx - 1 >= len(self._win_list):
            return
        w = self._win_list[idx - 1]
        self.mon.winfps.start(w["id"], w["name"])
        self._rebuild_inspector()

    def refreshWindows_(self, sender):
        self._rebuild_inspector()

    def openScreenAccess_(self, sender):
