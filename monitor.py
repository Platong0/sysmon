...
try:
    from winfps_mac import WindowFPS
    HAVE_WINFPS = True
...
class Monitor(NSObject):
    def init(self):
        ...
        self.winfps = WindowFPS.alloc().init() if HAVE_WINFPS else None
        self._winfps_last = 0
        self._winfps_fps = 0.0
    ...
    def refresh_(self, timer):
        ...
        # fps выбранного окна (ScreenCaptureKit): кадров за секунду
        if self.winfps is not None:
            wc = self.winfps.count
            self._winfps_fps = (wc - self._winfps_last) / dt
            self._winfps_last = wc
            if self.winfps.active and not self.winfps.error:
                data["winfps"] = [f"WFPS {self._winfps_fps:.0f}"]
                self.engine.values["winfps"] = self._winfps_fps
            elif self.winfps.error:
                data["winfps"] = ["WFPS —"]
            else:
                data["winfps"] = ["WFPS —"]
